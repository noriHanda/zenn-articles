...
Anthropicがコンテキストエンジニアリングのブログ記事で、この構造にprogressive disclosureという名前をつけています。もともとはUI設計の概念ですが、Anthropicはこれをエージェントの情報設計の中核原則として再定義しました。エージェントは必要な情報を必要なタイミングで段階的に取得し、理解をlayer by layerで組み立てていく。全情報を一度にコンテキストに詰め込むのではなく、関連するサブセットだけを保持する。ナレッジベースの設計にも同じことが言えます。生のドキュメントを丸ごと渡すのではなく、整理し、構造化し、文脈を付与した上で、そのタスクに必要な粒度の情報だけを渡す。
