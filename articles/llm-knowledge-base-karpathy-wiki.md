...
実際にある会社のGoogle Driveで試したところ、初回の整理だけでGoogle AgentSpace相当の検索精度が出ました。
...
| 得られた結果 | AgentSpace相当の検索精度、エンタープライズSaaSより安価に実現 |
...
| | 従来のRAG | LLM Wiki | Pinecone Nexus |
|------|------|------|------|
| 知識の加工タイミング | クエリ時（毎回） | ソース追加時（一度） | コンパイル時（一度） |
| LLMに渡すもの | 生のチャンク | 構造化されたWikiページ | タスク特化のartifact |
| 知識の蓄積 | しない | する（ページ間リンク） | する（artifact再利用） |
| 同じ質問への再回答 | 毎回検索し直す | 既存ページから即答 | 既存artifactから即答 |
